...
def parse_eaf_sentence(eaf_path, signer):
    """Extract (start_ms, end_ms) signing intervals from the Deutsche_Übersetzung_{signer} tier."""
...
def process_dgs_signer(keypoint_dir, eaf_dir, feature_type, signer, split_map):
    """Process all JSON files for one signer, returning seqs/labels/metadata dicts keyed by split."""
...
def process_dgs(dgs_dir_a, dgs_dir_b, eaf_dir, feature_type, split_map):
    ...
    Xa, ya, ma = process_dgs_signer(
        dgs_dir_a, eaf_dir, feature_type, "A", split_map)
    for s in ["train", "val", "test"]:
        seqs_s[s] += Xa[s]
        labs_s[s] += ya[s]
        metas_s[s] += ma[s]
    ...
    if dgs_dir_b is not None:
        ...
        Xb, yb, mb = process_dgs_signer(
            dgs_dir_b, eaf_dir, feature_type, "B", split_map)
        for s in ["train", "val", "test"]:
            seqs_s[s] += Xb[s]
            labs_s[s] += yb[s]
            metas_s[s] += mb[s]
