"""
Transformer sign language detection model.
...
class TransformerDetectorPerFrame(nn.Module):
    ...
    window_size (W): number of frames per attention window
    ...
    def _encode_window(self, x_win, lengths_win):
        ...
        # rows that are all padding get an unmasked key set, otherwise an
        # all-masked row plus the causal mask produces NaN in backward on ROCm
    ...
    def forward(self, x: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        # keep only the non-overlapping tail of each window (first window
        # keeps everything)
        start = 0
        first_window = True
        while start < T:
