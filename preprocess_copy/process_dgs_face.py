...
Preprocess DGS Corpus keypoint JSON files (body+hand+face) into per-frame
features and labels.
...
processing. Videos without a matching face JSON are skipped with a warning.
...
# ── EAF parsing (same as process_dgs.py) ─────────────────────────────────────
...
def process_dgs_signer_face(body_dir, face_dir, eaf_dir, feature_type,
                             signer, split_map, max_files=None):
    """
...
# ── Top-level DGS face processing ─────────────────────────────────────────────

def process_dgs_face(body_dir_a, face_dir_a, body_dir_b, face_dir_b,
                     eaf_dir, feature_type, split_map, max_files=None):
    ...
    Xa, ya, ma = process_dgs_signer_face(
        body_dir_a, face_dir_a, eaf_dir, feature_type, "A", split_map, max_files)
    for s in ["train", "val", "test"]:
        seqs_s[s] += Xa[s]
        labs_s[s] += ya[s]
        metas_s[s] += ma[s]
    ...
    if body_dir_b is not None:
        ...
        Xb, yb, mb = process_dgs_signer_face(
            body_dir_b, face_dir_b, eaf_dir, feature_type, "B", split_map, max_files)
        for s in ["train", "val", "test"]:
            seqs_s[s] += Xb[s]
            labs_s[s] += yb[s]
            metas_s[s] += mb[s]
...
# ── Split-map builder (identical logic to process_dgs.py) ────────────────────
...
def build_dgs_split_map(body_dir_a, body_dir_b, eaf_dir, seed=42):
    ...
    EAF filtering happens here so videos without annotations are never assigned
    to any split.
    """
...
def main():
    ...
    # Build and save split map (EAF-filtered, same seed as process_dgs.py).
    ...
    seqs_s, labs_s, metas_s = process_dgs_face(
        ...
        args.eaf_dir, args.feature_type, split_map, max_files,
    )
