...
Preprocess TED Talks keypoint JSON files (body+hand+face) into per-frame
features and labels.
...
Outlier filtering uses a combined criterion (TED-specific):
  - body+hand motion magnitude > MOTION_THRESHOLD  (p95)
    (face landmarks excluded to avoid noise from blinks / mouth movements)
  AND
  - shoulder jump > SHOULDER_JUMP_THRESHOLD  (p99)

Videos without a matching face JSON are skipped with a warning.
...
# ── TED-specific face sequence loader ────────────────────────────────────────
...
def load_ted_sequence_face(body_json_path, face_json_path, feature_type):
    ...
    Motion is computed on body+hand keypoints only (first BODY_HAND_N points)
    to avoid face landmark noise driving the filter.
    ...
    if features is None or len(body_frames) < 2:
        return None, None, fps, 0, len(body_frames) if body_frames else 0

    n_original = len(body_frames)

    # Rebuild coords_seq (body+hand only) for motion computation
...
def process_ted_face(body_dir, face_dir, feature_type, split_map,
    ...
    for i, body_path in enumerate(files, 1):
        ...
        print(f"  [{i}/{len(files)}] TED-face {split}: {body_path.name}",
              flush=True)
        try:
            features, _, _, n_removed, n_original = load_ted_sequence_face(
    ...
    if total_original > 0:
        ...
        print(f"\n  Outlier filtering (body+hand motion>p95 AND shoulder_jump>p99): "
...
def main():
    ...
    print(f"Outlier filter: body+hand motion > {MOTION_THRESHOLD} (p95) "
    ...
    seqs_s, labs_s, metas_s = process_ted_face(
        args.ted_dir, args.face_dir, args.feature_type, split_map, max_files)
