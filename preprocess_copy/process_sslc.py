...
Labels come from sentence-level EAF annotations (Översättning S1/S2 tier).
Videos with signing ratio < MIN_SIGNING_RATIO are excluded.
Frames before the first valid pose detection (disclaimer) are trimmed.

SSLC is recorded at 25 fps — no downsampling needed.
All data is output as a single test set (no train/val split).

Feature dims:
  optical_flow:  75  (per-keypoint L2 norm)
  raw_xy:       150  (75 × 2)
...
def parse_eaf_sentence(eaf_path, signer):
    """
    Extract signing intervals from Översättning S1/S2 tier.
...
# ── Sequence loading with disclaimer trimming ─────────────────────────────────
...
def load_sequence_sslc(json_path, feature_type):
    ...
    Subsequent invalid frames are forward-filled.

    Returns
    -------
    features        : float32 ndarray (T, 75) or (T, 150), or None
    T_valid         : int   — frame count after trimming
    fps             : float
    total_frames    : int   — original total frame count (before trimming)
    first_valid_idx : int   — frame index where valid pose first appears
    """
...
def load_ratio_map(ratio_csv):
    """Return dict: (video_id, signer) → signing_ratio float."""
    ratio_map = {}
    with open(ratio_csv, newline='') as f:
        for row in csv.DictReader(f):
            key = (row['video_id'].strip(), row['signer'].strip())
            ratio_map[key] = float(row['ratio'])
    return ratio_map


# ── Main processing ───────────────────────────────────────────────────────────

def process_sslc(pairs_file, keypoint_dir, eaf_dir, ratio_csv,
                 feature_type, max_files=None):

    ratio_map = load_ratio_map(ratio_csv)
    ...
    for i, row in enumerate(rows, 1):
        ...
        # Signing ratio filter
        ratio = ratio_map.get((video_id, signer), 0.0)
        if ratio < MIN_SIGNING_RATIO:
            skipped_ratio += 1
            continue
        ...
        print(f"  [{i}/{len(rows)}] {video_id} {signer} "
              f"(ratio={ratio*100:.1f}%)", flush=True)
    ...
    print(f"\nProcessed:       {len(seqs)}")
    print(f"Skipped (ratio): {skipped_ratio}")
...
def main():
    ...
    parser.add_argument('--ratio_csv',    required=True,
                        help="Path to sslc_signing_ratios.csv")
    ...
    parser.add_argument('--test_mode',    action='store_true',
                        help='Only process first 10 pairs')
    ...
    print(f"Min signing ratio: {MIN_SIGNING_RATIO*100:.0f}%")
    print(f"FPS:               25.0 (no downsampling)")
    print(f"{'[TEST MODE]' if args.test_mode else '[FULL MODE]'}\n")

    seqs, labs, metas = process_sslc(
        args.pairs_file, args.keypoint_dir, args.eaf_dir,
        args.ratio_csv, args.feature_type, max_files)
