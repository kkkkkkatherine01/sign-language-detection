...
Preprocess SSLC keypoint JSON files (body+hand+face) into per-frame features
...
# ── EAF parsing (identical to process_sslc.py) ────────────────────────────────
...
def load_sequence_sslc_face(body_json_path, face_json_path, feature_type):
    ...
    Returns
    -------
    features        : float32 ndarray (T, 167) or (T, 334), or None
    T_valid         : int
    fps             : float
    ...
    # Forward-fill subsequent invalid frames
...
def process_sslc_face(pairs_file, keypoint_dir, face_keypoint_dir,
                      eaf_dir, ratio_csv, feature_type, max_files=None):
...
def main():
    ...
    seqs, labs, metas = process_sslc_face(
        ...
        args.eaf_dir, args.ratio_csv, args.feature_type, max_files)
